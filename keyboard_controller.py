...
class KeyMapping(object):
	...
	##########################################################################################################
	# Adding extra keys for functionality
	Survey_Path	 = QtCore.Qt.Key.Key_V
	Trajectory_Path	 = QtCore.Qt.Key.Key_B
	Flight_Pause	 = QtCore.Qt.Key.Key_N
	Capture 	 = QtCore.Qt.Key.Key_O
	Drone_up         = QtCore.Qt.Key.Key_J
	Drone_down 	 = QtCore.Qt.Key.Key_M
        ###########################################################################################################
...
class KeyboardController(DroneVideoDisplay):
	def __init__(self):
		...
		self.pub_key = rospy.Publisher('key', String, queue_size = 300)
...
# We add a keyboard handler to the DroneVideoDisplay to react to keypresses
	def keyPressEvent(self, event):
		...
		if controller is not None and not event.isAutoRepeat():
			...
			###########################################################################################################
			# Code for conducting final project 
			elif key == KeyMapping.Survey_Path:
				self.key = "V"
				self.pub_key.publish(self.key)
			elif key == KeyMapping.Trajectory_Path:
				self.key = "B"
				self.pub_key.publish(self.key)
			elif key == KeyMapping.Flight_Pause:
				self.key = "N"
				self.pub_key.publish(self.key)
			elif key == KeyMapping.Drone_up:
				self.key = "J"
				self.pub_key.publish(self.key)
			elif key == KeyMapping.Drone_down:
				self.key = "M"
				self.pub_key.publish(self.key)		
			elif key == KeyMapping.Capture:
				super (KeyboardController, self).SaveImage()
			############################################################################################################
